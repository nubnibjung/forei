...
getData(pageLength = 40) {
  ...
  this.dataSubscription$ = this.reportService.LeadFollowupReport(this.filter).subscribe(
	...
	public sortingBy(event: IGo5TableStandardSortEvent) {
		...
		this.sortReportData(event.id, event.sortOrder?.toLowerCase() as 'asc' | 'desc')
	}
	private parseDate(dateStr: string): Date {
		// ตรวจจับรูปแบบ dd/MM/yyyy HH:mm
		...
		// fallback: ใช้ default Date parsing
		return new Date(dateStr)
	}
	public sortReportData(field: string, order: 'asc' | 'desc') {
		this.Reportdata.sort((a, b) => {
			...
			// ถ้าตัวใดตัวหนึ่งว่าง → เอาค่าว่างขึ้นบนเสมอ
			if (isEmptyA && !isEmptyB) return -1
			if (!isEmptyA && isEmptyB) return -1

			// ถ้าทั้งคู่ว่าง → ถือว่าเท่ากัน
			if (isEmptyA && isEmptyB) return 0

			// ถ้าเป็น date field
			if (field.toLowerCase().includes('date')) {
				const parsedDateA = this.parseDate(valueA)
				const parsedDateB = this.parseDate(valueB)
				return order === 'asc'
					? parsedDateA.getTime() - parsedDateB.getTime()
					: parsedDateB.getTime() - parsedDateA.getTime()
			}
